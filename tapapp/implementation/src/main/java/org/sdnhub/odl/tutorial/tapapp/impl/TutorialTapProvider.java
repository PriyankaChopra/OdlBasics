...
public class TutorialTapProvider implements AutoCloseable, DataChangeListener, OpendaylightInventoryListener {
    ...
    private List<Registration> registerDataChangeListeners() {
        ...
        try {
            ...
            TryTransactions tryTransactions = new TryTransactions(dataBroker);
            
            System.out.println("in PCEPTopologyProvider begining transactions --------------------------------------------");
            tryTransactions.writeTryObjectIntoDatastore();
            tryTransactions.readTryObjectFromDatastore();
            System.out.println("in PCEPTopologyProvider ending transactions --------------------------------------------");

            System.out.println("in PCEPTopologyProvider RPC registration --------------------------------------------");
            rpcProviderRegistry.addRpcImplementation(RpcYangService.class,new RpcYangServiceImpl());

            System.out.println("in PCEPTopologyProvider RPC calling --------------------------------------------");
            RpcYangService testRpc = rpcProviderRegistry.getRpcService(RpcYangService.class);
            RpcResult<CheckContainerRpcOutput> result = testRpc.checkContainerRpc(null).get();
            System.out.println(result.getResult().isStatus());
            
            TryNotifications tryNotifications =  new TryNotifications(notificationProviderService);
            tryNotifications.registerForNoification();
            tryNotifications.publishNotification();
